...
export function UploadModal({
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop,
    // ✅ Fixed PDF accept — mime type only, no extension needed
    accept: {
      "image/*": [],
      "application/pdf": [],
    },
  ...
  const uploadFiles = async (files: File[]) => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        // ✅ FIX 1 — Removed manual Content-Type (Axios handles it for FormData)
        // ✅ FIX 2 — Updated endpoint from /upload to /api/upload
      ...
      <div className="relative z-10 w-[min(900px,95%)] bg-card rounded-lg shadow-lg overflow-hidden">
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <div
              {...getRootProps()}
              className={`bg-blue border-2 border-dashed p-6 rounded-md text-center cursor-pointer ${
                isDragActive ? "border-primary/80 bg-primary/5" : "border-muted"
              }`}
              style={{ minHeight: 220 }}
            >
              <input {...getInputProps()} />
          ...
          <div>
            <div className="rounded-md border p-2 flex flex-col items-center justify-center">
              ...
              <div className="text-xs text-muted-foreground mt-2">
                Use the camera to capture an image of a document.
              </div>
            </div>
          </div>
        </div>
